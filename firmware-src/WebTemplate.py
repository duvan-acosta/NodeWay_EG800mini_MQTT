...
class WebTemplate:
    ...
    def __init__(self, code, escapeStrFunc=None, filepath=""):
        ...
        self._rendered = ""
        self._instructions = {
            WebTemplate.INSTRUCTION_PYTHON: self._processInstructionPYTHON,
            WebTemplate.INSTRUCTION_IF: self._processInstructionIF,
            WebTemplate.INSTRUCTION_ELIF: self._processInstructionELIF,
            WebTemplate.INSTRUCTION_ELSE: self._processInstructionELSE,
            WebTemplate.INSTRUCTION_FOR: self._processInstructionFOR,
            WebTemplate.INSTRUCTION_END: self._processInstructionEND,
            WebTemplate.INSTRUCTION_INCLUDE: self._processInstructionINCLUDE,
        }
    ...
    def _parseCode(self, pyGlobalVars, pyLocalVars, execute):
        ...
        self._pyLocalVars["MESSAGE_TEXT"] = WebTemplate.MESSAGE_TEXT
        self._pyLocalVars["MESSAGE_STYLE"] = WebTemplate.MESSAGE_STYLE
        self._rendered = ""
        ...
        WebTemplate.MESSAGE_TEXT = ""
        WebTemplate.MESSAGE_STYLE = ""

    def _parseBloc(self, execute):
        while self._pos <= self._endPos:
            c = self._code[self._pos]
            if (
                c == WebTemplate.TOKEN_OPEN[0]
                and self._code[self._pos : self._pos + WebTemplate.TOKEN_OPEN_LEN]
                == WebTemplate.TOKEN_OPEN
            ):
                self._pos += WebTemplate.TOKEN_OPEN_LEN
                tokenContent = ""
                x = self._pos
                while True:
                    if x > self._endPos:
                        raise Exception(
                            "%s is missing (line %s)"
                            % (WebTemplate.TOKEN_CLOSE, self._line)
                        )
                    c = self._code[x]
                    if (
                        c == WebTemplate.TOKEN_CLOSE[0]
                        and self._code[x : x + WebTemplate.TOKEN_CLOSE_LEN]
                        == WebTemplate.TOKEN_CLOSE
                    ):
                        self._pos = x + WebTemplate.TOKEN_CLOSE_LEN
                        break
                    elif c == "\n":
                        self._line += 1
                    tokenContent += c
    ...
    def _processToken(self, tokenContent, execute):
        ...
        if len(instructName) == 0:
            raise Exception(
                ...
                % (WebTemplate.TOKEN_OPEN, WebTemplate.TOKEN_CLOSE, self._line)
            )
    ...
    def _processInstructionPYTHON(self, instructionBody, execute):
        if instructionBody is not None:
            raise Exception(
                ...
                % (WebTemplate.INSTRUCTION_PYTHON, self._line)
            )
        pyCode = ""
        while True:
            if self._pos > self._endPos:
                raise Exception(
                    ...
                    % (WebTemplate.INSTRUCTION_END, self._line)
                )
            c = self._code[self._pos]
            if (
                c == WebTemplate.TOKEN_OPEN[0]
                ...
                while True:
                    ...
                    x += 1
                tokenContent = tokenContent.strip()
                if tokenContent == WebTemplate.INSTRUCTION_END:
                    break
                raise Exception(
    ...
    def _processInstructionIF(self, instructionBody, execute):
        if instructionBody is not None:
            ...
            if newTokenToProcess is not None:
                if newTokenToProcess == WebTemplate.INSTRUCTION_END:
                    return None
                elif newTokenToProcess == WebTemplate.INSTRUCTION_ELSE:
                ...
                elif newTokenToProcess == WebTemplate.INSTRUCTION_ELIF:
        ...
        raise Exception(
            ...
            % (WebTemplate.INSTRUCTION_IF, self._line)
        )
    ...
    def _processInstructionELIF(self, instructionBody, execute):
        if instructionBody is None:
            raise Exception(
                '"%s" alone is an incomplete syntax (line %s)'
                % (WebTemplate.INSTRUCTION_ELIF, self._line)
            )
        self._elifInstructionBody = instructionBody
        return WebTemplate.INSTRUCTION_ELIF

    def _processInstructionELSE(self, instructionBody, execute):
        if instructionBody is not None:
            raise Exception(
                'Instruction "%s" is invalid (line %s)'
                % (WebTemplate.INSTRUCTION_ELSE, self._line)
            )
        return WebTemplate.INSTRUCTION_ELSE
    ...
    def _processInstructionFOR(self, instructionBody, execute):
        if instructionBody is not None:
            ...
            raise Exception(
                ...
                % (WebTemplate.INSTRUCTION_FOR, instructionBody)
            )
        raise Exception(
            '"%s" alone is an incomplete syntax (line %s)'
            % (WebTemplate.INSTRUCTION_FOR, self._line)
        )
