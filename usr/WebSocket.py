...
class WebSocket:
    ...
    def __init__(
        ...
        if self._handshake(httpResponse):
            self._ctrlBuf = WebSocket._tryAllocByteArray(0x7D)
            self._msgBuf = WebSocket._tryAllocByteArray(maxRecvLen)
            if self._ctrlBuf and self._msgBuf:
                self._msgType = None
                self._msgLen = 0
                if threaded:
                    if WebSocket._tryStartThread(self._wsProcess, (acceptCallback,)):
                        return
                else:
                    self._wsProcess(acceptCallback)
                    return
            print("WebSocket : Out of memory on new WebSocket connection.")
        try:
    ...
    def _handshake(self, httpResponse):
        print(self._httpCli.GetRequestHeaders())
        try:
    ...
    def _wsProcess(self, acceptCallback):
        ...
        try:
            ...
            print("WebSocket : Error on accept callback (%s)." % str(ex))
        ...
        if self.ClosedCallback:
            try:
                ...
                print("WebSocket : Error on closed callback (%s)." % str(ex))
                        ...
                        if self._msgType == self._msgTypeText:
                            if self.RecvTextCallback:
                                try:
                                    ...
                                    print(
                                        "WebSocket : Error on recv text callback (%s)."
                                        % str(ex)
                                    )
                        else:
                            if self.RecvBinaryCallback:
                                try:
                                    ...
                                    print(
                                        "WebSocket : Error on recv binary callback (%s)."
                                        % str(ex)
                                    )
                        self._msgType = None
                        self._msgLen = 0
